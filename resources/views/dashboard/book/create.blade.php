...
<div class="grid grid-cols-12 gap-4">
  <div class="col-span-12 lg:col-span-9 p-4">
    <div class="bg-white p-6 rounded-lg shadow">
      <h2 class="text-2xl font-semibold text-gray-800 mb-6">Add Book</h2>
      <form action="/dashboard/book" method="POST" class="space-y-6" enctype="multipart/form-data">
        ...
        <!-- Published At Field -->
        <div>
            <label for="publication_at" class="block text-sm font-medium text-gray-700">Published At</label>
            <input type="datetime-local" name="publication_at" id="publication_at"
            value="{{ old('publication_at', isset($book->publication_at) ? $book->publication_at->format('Y-m-d\TH:i') : '') }}"
            required
            class="mt-1 p-2 block w-full rounded-md border border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('publication_at') border-red-500 @enderror">
            @error('publication_at')
            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
